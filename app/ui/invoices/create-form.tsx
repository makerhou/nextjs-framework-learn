...
export default function Form({customers}: { customers: CustomerField[] }) {
    const initialValues: any = {message: null, error: {}}
    const [state, dispatch] = useFormState(createInvoice as any, initialValues);
    return (
        <form action={dispatch}>
            <div className="rounded-md bg-gray-50 p-4 md:p-6">
                ...
                <div className="mb-4">
                    {
                        state.message  && (
                                <div key={state.message} className="text-red-600 text-sm">
                                    {state.message}
                                </div>
                            )
                    }
